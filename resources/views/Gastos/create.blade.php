...
@section('content')
    ...
    <div class="block mx-auto my-40 p-8 bg-white w-1/3 border border-gray-200 rounded-lg shadow-lg">
        ...
        <p class="text-5xl mb-14 mt-10 text-center font-bold">Gastos</p>
        @if($errors->has('value'))
        ...
        @endif
        <form action="{{route('gasto.store')}}" class="mt-8" method="POST" enctype="multipart/form-data">
            @csrf

            <input type="number" name="value" placeholder="valor"
            id="value" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('value')}}">

            <input type="text" name="description" placeholder="Decripcion"
            id="description" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('description')}}">

            <input type="date" name="date" placeholder="Fecha"
            id="date" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('date')}}">
