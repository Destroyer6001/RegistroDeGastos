...
@section('content')
    <div class="block mx-auto my-40 p-8 bg-white w-1/3 border border-gray-200 rounded-lg shadow-lg">
        ...
        <p class="text-5xl mb-14 mt-10 text-center font-bold">Editar Categoria</p>
        @if($errors->has('name'))
        ...
        @endif
        <form action="{{route('categoria.update',['id' => $category->id])}}" class="mt-8" method="POST">
            @csrf
            @method('PATCH')
            <input type="text" name="name" placeholder="nombre"
            id="name" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{$category->name}}">
            
            <input type="text" name="description" placeholder="descripcion"
            id="description" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{$category->description}}">
